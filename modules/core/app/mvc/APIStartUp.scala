...
abstract class APIStartUp(authUserId: RequestHeader => Option[Long] = Unit => None) extends BaseAPIController {
   ...
   */
  def APIAction(f: Request[AnyContent] => Result): Action[AnyContent] = Action { implicit request =>
   ...
   * @return Action[AnyContent] Play action object.
   */
  def APIActionWithAuth(f: => Long => (Request[AnyContent]) => Result): Action[AnyContent] = Action { implicit request =>
    val start = System.currentTimeMillis
